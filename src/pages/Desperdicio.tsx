...
export default function Desperdicio() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Registrar desperdício via função segura (bypass RLS interno)
      const { data: wasteId, error: rpcError } = await (supabase as any).rpc('register_waste', {
        p_product_id: formData.product_id,
        p_user_id: user?.id ?? null,
        p_quantidade: parseInt(formData.quantidade),
        p_motivo: formData.motivo || null,
        p_image_paths: imagePaths.length > 0 ? imagePaths : null,
      });

      if (rpcError) {
        console.error('register_waste RPC error:', rpcError);
        throw rpcError;
      }
